...
def startup_animation(callback):
    ...
    pygame.init()
    pygame.mixer.init()  # mixer hatasını önlemek için
    WIDTH, HEIGHT = 900, 600
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("BTL Başlatılıyor")
    ...
    # GIF yükleme (Pillow ile frame -> pygame Surface dönüşümü)
    gif_path = r"C:\Users\Yiğit Aslan\Desktop\BTL_setups.exe\btlload\Loading.gif"
    gif_frames = []
    gif_cumulative = []
    total_gif_duration = 0

    if Image is not None and os.path.exists(gif_path):
        try:
            ...
            for frame in ImageSequence.Iterator(img):
                ...
                d = frame.info.get('duration', 100)  # ms
                if d <= 0:
                    d = 100
                durations.append(d)
            # cumulative durations for easy frame lookup
            cum = 0
            for d in durations:
                cum += d
        ...
        # fallback: try pygame to load a static image (won't animate)
        try:
            if os.path.exists(gif_path):
                surf = pygame.image.load(gif_path).convert_alpha()
                gif_frames = [surf]
                gif_cumulative = [100]
                total_gif_duration = 100
        except Exception:
            gif_frames = []
            gif_cumulative = []
            total_gif_duration = 0
    ...
    while running:
        ...
        progress_pct = min(1.0, elapsed/duration)

        # animate text border + fill
        pulse = 128 + int(127 * (1 + random.random()) * 0.5)
        text_surface = font.render(text, True, (pulse, pulse, 255))  # içi açık
        border_surface = font.render(text, True, (0,0,50))  # border koyu

        offsets = [(-2,0),(2,0),(0,-2),(0,2)]
        for ox, oy in offsets:
            screen.blit(border_surface, (WIDTH//2 - text_surface.get_width()//2 + ox,
                                         HEIGHT//2 - text_surface.get_height()//2 + oy))
        screen.blit(text_surface, (WIDTH//2 - text_surface.get_width()//2,
                                   HEIGHT//2 - text_surface.get_height()//2))

        # <<< CYAN TOPLAR KALDIRILDI: BURASI BOŞ >>>

        # GIF gösterimi (progress bar yerine)
        if gif_frames:
            # GIF zamanlamasına göre hangi frame gösterilecek hesaplanır
            gif_ms = int((elapsed * 1000))  # milisaniye
            if total_gif_duration > 0:
                ...
                frame_idx = int((gif_ms // 100) % len(gif_frames))
            frame_img = gif_frames[frame_idx]
            # GIF'i orijinal progress barın bulunduğu alana yakın olacak şekilde alt tarafa yerleştir
            gif_rect = frame_img.get_rect(center=(WIDTH//2, HEIGHT - 100 + 12))
            screen.blit(frame_img, gif_rect)
...
def open_cmd_panel():
    ...
    scroll = ttk.Scrollbar(win, command=output_text.yview)
        ...
        def task():
            try:
                ...
                for line in proc.stdout:
                    ...
                    output_text.see("end")
                proc.wait()
                output_text.insert(END, f"[exit {proc.returncode}]\n")
            except Exception as e:
                output_text.insert(END, f"Hata: {e}\n")
        threading.Thread(target=task, daemon=True).start()
        entry.delete(0, END)
    run_btn = Button(ctrl, text="Çalıştır", command=run)
